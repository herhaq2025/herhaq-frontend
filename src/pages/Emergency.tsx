...
//     const saved = localStorage.getItem('haq-emergency-contacts');
...
//     return localStorage.getItem('haq-trigger-phrase') || 'I want mangoes';
//   });
...
//   useEffect(() => {
//     localStorage.setItem('haq-emergency-contacts', JSON.stringify(emergencyContacts));
//   }, [emergencyContacts]);

//   // Save trigger phrase to localStorage whenever it changes
//   useEffect(() => {
//     localStorage.setItem('haq-trigger-phrase', triggerPhrase);
//   }, [triggerPhrase]);
...
const Emergency: React.FC = () => {
  ...
  const [userPhone, setUserPhone] = useState(() => {
    return localStorage.getItem('haq-user-phone') || '';
  });
  const [newContact, setNewContact] = useState({ name: '', phone: '', relationship: '' });
  const { language } = useLanguage();
  ...
  // Save user phone to localStorage whenever it changes
  useEffect(() => {
    localStorage.setItem('haq-user-phone', userPhone);
  }, [userPhone]);
  ...
  const triggerEmergencyAlert = () => {
    if (!userPhone.trim()) {
      alert(language === 'en' 
        ? 'Please set your phone number first in the settings below.'
        : 'براہ کرم پہلے نیچے سیٹنگز میں اپنا فون نمبر سیٹ کریں۔'
      );
      return;
    }
    ...
    if (emergencyContacts.length === 0) {
      alert(language === 'en' 
        ...
        : 'براہ کرم پہلے ایمرجنسی رابطے شامل کریں۔'
      );
      return;
    }
    ...
    const alertMessage = `🚨 EMERGENCY ALERT 🚨%0A%0AI need immediate help!%0A%0AFrom: ${userPhone}%0A%0A${locationText}%0A%0APlease contact me or authorities immediately.%0A%0ATime: ${new Date().toLocaleString()}`;
    
    // Send to each emergency contact
    emergencyContacts.forEach(contact => {
      const whatsappUrl = `https://api.whatsapp.com/send?phone=${contact.phone.replace(/\D/g, '')}&text=${alertMessage}`;
      window.open(whatsappUrl, '_blank');
    });
  ...
  const testEmergencySystem = () => {
    if (!userPhone.trim()) {
      alert(language === 'en' 
        ? 'Please set your phone number first.'
        : 'براہ کرم پہلے اپنا فون نمبر سیٹ کریں۔'
      );
      return;
    }

    const testMessage = `🧪 TEST ALERT 🧪%0A%0AThis is a test of the emergency system.%0A%0AFrom: ${userPhone}%0A%0AIf you receive this message, the system is working correctly.%0A%0ATime: ${new Date().toLocaleString()}`;
    
    const whatsappUrl = `https://api.whatsapp.com/send?phone=${emergencyContacts[0]?.phone.replace(/\D/g, '')}&text=${testMessage}`;
    window.open(whatsappUrl, '_blank');
  };
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          >
            ...
            <div className="bg-white rounded-2xl shadow-lg p-8">
              <div className="flex items-center gap-3 mb-6">
                <Phone className="h-8 w-8 text-[#F06292]" />
                <h3 className="text-2xl font-bold text-[#333333]">
                  {language === 'en' ? 'Your Phone Number' : 'آپ کا فون نمبر'}
                </h3>
              </div>
              
              <p className="text-[#333333] mb-4">
                {language === 'en' 
                  ? 'Set your phone number so emergency contacts know who is sending the alert'
                  : 'اپنا فون نمبر سیٹ کریں تاکہ ایمرجنسی رابطوں کو معلوم ہو کہ الرٹ کون بھیج رہا ہے'
                }
              </p>
              ...
              <div className="p-4 bg-[#FAFAFA] rounded-lg">
                <p className="text-sm text-[#333333]">
                  <strong>{language === 'en' ? 'Note:' : 'نوٹ:'}</strong> {language === 'en' 
                    ? 'This number will be included in emergency messages so contacts know who needs help.'
                    : 'یہ نمبر ایمرجنسی پیغامات میں شامل کیا جائے گا تاکہ رابطوں کو معلوم ہو کہ کسے مدد کی ضرورت ہے۔'
                  }
                </p>
              </div>
            </div>
            ...
            <div className="bg-white rounded-2xl shadow-lg p-8">
              ...
              <Button
                ...
                onClick={testEmergencySystem}
                className="w-full"
                disabled={emergencyContacts.length === 0 || !userPhone.trim()}
              >
                {language === 'en' ? 'Send Test Alert' : 'ٹیسٹ الرٹ بھیجیں'}
              </Button>
              
              {(!userPhone.trim() || emergencyContacts.length === 0) && (
                <p className="text-sm text-gray-500 mt-2 text-center">
                  {language === 'en' 
                    ? 'Please set your phone number and add emergency contacts to test the system'
                    : 'سسٹم ٹیسٹ کرنے کے لیے براہ کرم اپنا فون نمبر سیٹ کریں اور ایمرجنسی رابطے شامل کریں'
                  }
                </p>
              )}
            </div>
